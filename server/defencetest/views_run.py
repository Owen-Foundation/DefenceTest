...
from defencetest.spsa_workflow import build_spsa_state
...
from defencetest.views_helpers import _host_url
...
def get_nets(commit_sha: str, repo_url: str) -> list[str]:  # noqa: C901
    """Retrieve default net filename from the Owen uci option table."""
    try:
        nets = []
        pattern = re.compile(r"nn-[a-f0-9]{12}\.o2nn")
...
def validate_form(request: Any) -> dict[str, Any]:  # noqa: ANN401, C901, PLR0912, PLR0915
    ...
    # Deal with people that have forked from "mcostalba/Owen" instead
    # of from "Owen-Foundation/Owen".
    official_repo = "https://github.com/Owen-Foundation/Owen"
    ...
    from defencetest.schemas import tc as tc_schema  # noqa: PLC0415
    ...
    if stop_rule == "sprt":
        ...
        if elo_model not in ["BayesElo", "logistic", "normalized"]:
            ...
            raise ValueError(msg)
        data["sprt"] = defencetest.stats.stat_util.SPRT(
            alpha=0.05,
            beta=0.05,
