"""Dispatch defencetest UI routes and preserve the legacy request contract.
...
from defencetest.http.csrf import csrf_token_from_form
...
from defencetest.http.open_graph import (
...
from defencetest.http.template_helpers import (
...
from defencetest.http.template_renderer import render_template_to_response
...
from defencetest.schemas import (
    RUN_VERSION,
    github_repo_input,
    is_undecided,
    runs_schema,
...
from defencetest.spsa_workflow import build_spsa_form_values, format_spsa_value
from defencetest.util import (
...
from defencetest.views_actions import actions as _actions_impl
from defencetest.views_finished import get_paginated_finished_runs
...
from defencetest.views_machines import tests_machines as _tests_machines_impl
...
def login(request: _ViewContext) -> dict[str, Any] | RedirectResponse:
    ...
    if request.method == "POST":
        ...
        if token.get("error_code") == "pending":
            message += (
                " If you recently registered to defencetest, "
...
Owen. Unfortunately, it seems your DefenceTest \
...
any help, or else open an issue at \
https://github.com/Owen-Foundation/DefenceTest/issues

Enjoy your day,

{blocker_name} (DefenceTest approver)
...
def upload(request: _ViewContext) -> dict[str, Any] | RedirectResponse:  # noqa: C901, PLR0911, PLR0912, PLR0915
    ...
    base_context = {
        ...
        "testing_guidelines_url": "https://github.com/Owen-Foundation/DefenceTest/wiki/Creating-my-first-test",
    ...
    if not re.match(r"^nn-[0-9a-f]{12}\.o2nn$", filename):
        errors.append('Name must match "nn-[SHA256 first 12 digits].o2nn"')
...
def tests_finished(request: _ViewContext) -> dict[str, Any] | Response:
    ...
    if search_mode:
        title_text = f"Search Finished Tests - page {page_idx + 1} | DefenceTest"
    else:
        title_text = (
            f"Finished Tests{title_suffix} - page {page_idx + 1} | DefenceTest"
        )
    context_out = {
        **context,
...
def _build_live_elo_context(run: dict[str, Any]) -> dict[str, Any]:
    ...
    a = defencetest.stats.stat_util.SPRT_elo(
        results,
        alpha=sprt["alpha"],
        beta=sprt["beta"],
        elo0=sprt["elo0"],
...
def tests_view(request: _ViewContext) -> dict[str, Any] | RedirectResponse:  # noqa: C901, PLR0912, PLR0915
    ...
    if "master_repo" in run["args"]:  # if present then it is non-standard
        warnings.append(
            "the developer repository is not forked from Owen-Foundation/Owen",
        )
