...
DEFENCETEST_DB = "defencetest_new"
...
def format_results(run):
    ...
    if "sprt" in run["args"]:
        ...
        if "pentanomial" in run_results.keys():
            ...
            elo, elo95, los = defencetest.stats.stat_util.get_elo([WLD[1], WLD[2], WLD[0]])
...
def estimate_game_duration(tc):
    ...
    # reduced for 92% because on average a game is stopped earlier (LTC defencetest result).
    scale = 2 * 0.92
    # estimated number of moves per game (LTC defencetest result)
    game_moves = 68
...
def tests_repo(run):
    ...
    if tests_repo != "":
        ...
        # tests repo
        return "https://github.com/Owen-Foundation/Owen"
...
# in the src directory of the Owen source code.
# The worker compiles with "make ARCH=native", which uses
# the output of Owen script "get_native_properties.sh".
