...
Expose the production entrypoint ``uvicorn defencetest.app:app`` and manage
...
Keep request and response behavior in ``defencetest.http``.
"""
...
from defencetest import schemas
from defencetest.api import router as api_router
from defencetest.http.cookie_session import (
    DEFAULT_SAMESITE,
    SESSION_COOKIE_NAME,
    session_secret_key,
)
from defencetest.http.errors import install_error_handlers
from defencetest.http.middleware import (
...
from defencetest.views import router as views_router
    ...
    try:
        if rundb.port >= 0:
            await run_in_threadpool(
                ...
                message=f"stop defencetest@{rundb.port}",
            )
    except Exception:
...
def create_app() -> FastAPI:
    ...
    #   OPENAPI_URL=/openapi.json uvicorn defencetest.app:app --reload
    ...
    app.add_middleware(
        cast("MiddlewareFactory", DefenceTestSessionMiddleware),
