...
from defencetest.schemas import user_schema
...
def normalize_user(user):
    # PyMongo returns datetimes as timezone-naive; the schema requires
    # UTC-aware. Re-attach UTC so re-validating a fetched user succeeds.
    rt = user.get("registration_time")
    if rt is not None and rt.tzinfo is None:
        user["registration_time"] = rt.replace(tzinfo=UTC)
    return user
...
class UserDb:
    ...
    def add_user_group(self, username, group):
        ...
        validate_user(normalize_user(user))
