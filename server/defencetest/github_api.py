...
from defencetest.schemas import sha as sha_schema
...
def _download_from_github_raw(
    item, user="Owen-Foundation", repo="Owen", branch="master"
):
...
def compare_sha(
    ...
    # See e.g.
    # https://YOUR_DEFENCETEST_HOST/actions?max_count=1&before=1767274733.476365
    ...
    url = (
        "https://api.github.com/repos/Owen-Foundation/"
        f"Owen/compare/{user1}:{sha1}...{user2}:{sha2}"
    )
    r = call(
        url,
...
def get_master_repo(
    user="Owen-Foundation", repo="Owen", ignore_rate_limit=False
):
...
def compare_branches_url(
    user1="owen-chess", branch1="master", user2=None, branch2=None
):
    if user2 is None:
        user2 = user1
    return (
...
def commit_url(user="Owen-Foundation", repo="Owen", branch="master"):
