...
import defencetest.run_cache
import defencetest.spsa_handler
import defencetest.stats.stat_util
from defencetest.actiondb import ActionDb
from defencetest.http.settings import TASK_SEMAPHORE_SIZE
from defencetest.kvstore import KeyValueStore
from defencetest.lru_cache import lru_cache
from defencetest.run_cache import Prio
from defencetest.scheduler import Scheduler
...
from defencetest.stats.stat_util import SPRT_elo
...
class RunDb:
    def __init__(self, db_name=DEFENCETEST_DB, port=-1, is_primary_instance=True):
        ...
        if self.port >= 0:
            self.actiondb.system_event(message=f"start defencetest@{self.port}")

        self.__is_primary_instance = is_primary_instance

        self.run_cache = defencetest.run_cache.RunCache(self.runs)
        self.active_run_lock = self.run_cache.active_run_lock
        if is_primary_instance:
            self.buffer = self.run_cache.buffer
        url = os.getenv("DEFENCETEST_URL")
        ...
        self.spsa_handler = defencetest.spsa_handler.SPSAHandler(self)
    ...
    def validate_data_structures(self):
        ...
        print(
            "Validate_data_structures: validating DefenceTest's internal data structures...",
            flush=True,
        )
        try:
    ...
    @staticmethod
    def _get_unfinished_runs_query(username=None):
        unfinished_runs_query = {"finished": False}
        if username:
            unfinished_runs_query["args.username"] = username
        return unfinished_runs_query

    def get_unfinished_runs(self, username=None):
        # Note: the result can be only used once.

        unfinished_runs = self.runs.find(
            self._get_unfinished_runs_query(username),
            _UNFINISHED_RUNS_LIGHTWEIGHT_PROJECTION,
        )
        return unfinished_runs
...
If you require help, feel free to open an issue at
    ...
    def sync_request_task(self, worker_info):
        ...
        for run in unfinished_runs:
            ...
            if arch_filter != "":
                ...
                # arch string.
                # See https://github.com/Owen-Foundation/DefenceTest/pull/2428#issuecomment-3715147268
                try:
    ...
    def finished_run_message(self, run):
        ...
        if "sprt" in run["args"]:
            ...
            if "pentanomial" in results:
                elo, elo95, los = defencetest.stats.stat_util.get_elo(
                    results["pentanomial"]
                )
            else:
    ...
    def purge_run(self, run, p=0.001, res=7.0, iters=1):
        ...
        if message == "":
            ...
            if "sprt" in run["args"] and "state" in run["args"]["sprt"]:
                defencetest.stats.stat_util.update_SPRT(results, run["args"]["sprt"])
