...
# ${VENV}/bin/aws s3 ls s3://defencetest/backup/archive/
# Download a backup with:
# ${VENV}/bin/aws s3 cp s3://defencetest/backup/archive/<YYYYMMDD>/dump.tar dump.tar

# Load the variables with the AWS keys, cron uses a limited environment
. ${HOME}/.profile

cd ${HOME}/backup
for db_name in "defencetest_new" "admin" "config" "local"; do
...
${VENV}/bin/aws s3 cp dump.tar s3://defencetest/backup/archive/${date_utc}/dump.tar
